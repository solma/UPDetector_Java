...
public class WavFile
{
	...
	// @ pos: start position in buffer, numBytes: no. of bytes to read
	// ------------------------------------------------
	...
	public static double[] readAllFrames(String filename){
		try
		{
			...
			final int BUF_SIZE = (int)numFrames;//5001;
...
//			long[] buffer = new long[BUF_SIZE * numChannels];
			...
			int framesRead=readWavFile.readFrames(buffer, BUF_SIZE);				
			
			//copy the buffer to frameData
			double[] frameData=new double[(int)numFrames];
			//System.out.println("floatScale"+ readWavFile.floatScale);
			for(int i=0;i<framesRead;i++){
				//normalize 
				frameData[i]=buffer[i]*readWavFile.floatScale/numFrames;
			}
			int n=frameData.length;
			System.out.println(Arrays.toString(Arrays.copyOfRange(frameData, n-10, n)));
	...
	public void main(){
		String[] inputFiles={"snoring"} ;//, "ted-talk"};

		try
		{
			for (String filename : inputFiles)
			{
				filename=Constants.AUDIO_BASE_DIR+filename+".wav";
				WavFile readWavFile = openWavFile(new File(filename));
				readWavFile.display();
				...
				WavFile writeWavFile = newWavFile(new File(Constants.AUDIO_BASE_DIR+"out.wav"), numChannels, numFrames, validBits, sampleRate);
...
//				long[] buffer = new long[BUF_SIZE * numChannels];
				...
				do
				{
					...
					System.out.printf("read %d frames, write %d frames\n",  framesRead, framesWritten);
					cnt[0]+=framesRead; cnt[1]+=framesWritten;
				}while (framesRead != 0);
				System.out.printf("total read %d frames, total write %d frames\n", cnt[0], cnt[1]);
				
				readWavFile.close();
				writeWavFile.close();
			}

			WavFile writeWavFile = newWavFile(new File(Constants.AUDIO_BASE_DIR+"out2.wav"), 1, 10, 23, 44100);
	...
	{
		new WavFile().main();
		//String inputAudioFile=Constants.AUDIO_BASE_DIR+"snoring"+".wav";
		//double[] data= WavFile.readAllFrames(inputAudioFile) ;
	}
	
	public static void writeMonoFile(String name, double sampleRate, double[] data)
	{
		...
		try{
			...
			WavFile wavFile = WavFile.newWavFile(new File(name), 1, numFrames, 16, (long) sampleRate);
			...
			while (frameCounter < numFrames)
			{
				...
				// Write the buffer
				wavFile.writeFrames(data, frameCounter, toWrite);
